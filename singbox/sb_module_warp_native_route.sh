...
# ============================================================
#  Sing-box Native WARP 管理模块 (v4.3 Ultimate - Final Polish)
#  - 架构: Endpoint (v1.12+)
#  - 核心: Systemd/Pkill 双模兼容 + 自动回滚保护
#  - 策略: 2025 全面流媒体规则 (Netflix/Disney+/HBO/Paramount+/Spotify)
# ============================================================
...
write_warp_config() {
    ...
    echo -e "${YELLOW}正在应用 v4.3 Endpoint 架构配置...${PLAIN}"
    ...
    if [[ $? -eq 0 && -s "$TMP_CONF" ]]; then
        ...
        echo -e "${GREEN}配置写入成功，正在验证并重启...${PLAIN}"
        restart_sb
    else
...
ensure_warp_exists() {
    # 如果 config 中已有 WARP Endpoint，直接返回成功
    if jq -e '.endpoints[]? | select(.tag == "WARP")' "$CONFIG_FILE" >/dev/null 2>&1; then
        return 0
    fi

    # 如果没有 Endpoint，但有凭证文件 → 自动恢复
    if [[ -f "$CRED_FILE" ]]; then
        source "$CRED_FILE" 2>/dev/null
        if [[ -n "$PRIV_KEY" && -n "$V6_ADDR" ]]; then
            echo -e "${YELLOW}检测到已有凭证，自动恢复 WARP Endpoint 配置...${PLAIN}"
            write_warp_config "$PRIV_KEY" "$PUB_KEY" "$V4_ADDR" "$V6_ADDR" "$RESERVED"
            return 0
        fi
    fi

    echo -e "${RED}错误: 未找到 WARP 配置或有效凭证，请先配置凭证。${PLAIN}"
    return 1
}
...
mode_stream() {
    ...
    # 2025 Revised: 纯域名规则 (修复了 Spotify URL 错误) + HBO/Paramount
    apply_routing_rule "$(jq -n '{
        "domain_suffix": [
            "netflix.com","nflxvideo.net","netflix.net",
            "openai.com","chatgpt.com","ai.com",
            "google.com","youtube.com","youtu.be",
            "disneyplus.com","hulu.com",
            "hbonow.com","hbomax.com","paramountplus.com",
            "googleusercontent.com","spotifycdn.com","scdn.co","pscp.tv"
        ], 
        "outbound": "WARP" 
    }')"
    echo -e "${GREEN}流媒体分流策略已应用 (2025 Final)。${PLAIN}"
}
...
mode_specific_node() {
    ...
    [[ -z "$selection" ]] && { echo -e "${RED}未选择节点。${PLAIN}"; return; }
...
show_menu() {
    ...
    while true; do
        ...
        if [[ -f "$CONFIG_FILE" ]] && jq -e '.endpoints[]? | select(.tag == "WARP")' "$CONFIG_FILE" >/dev/null 2>&1; then
...
auto_main() {
    echo -e "${GREEN}>>> [WARP-SB] 启动自动化部署 (v4.3)...${PLAIN}"
    check_dependencies
