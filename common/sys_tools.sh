...
configure_ssh_security() {
    ...
    clear
    echo -e "${BLUE}========= SSH 安全加固 (公钥/禁用密码/修改端口) =========${PLAIN}"
    echo -e "1. 导入公钥: 解决 VPS 重装后需要反复输入密码的烦恼。"
    echo -e "2. 禁用密码: 彻底杜绝 SSH 暴力破解，提升安全等级。"
    echo -e "3. 修改端口: 修改默认的 22 端口，规避扫描。"
    echo -e "------------------------------------------------"
    ...
    echo -e "\n${YELLOW}第三步: 修改 SSH 监听端口 (可选)${PLAIN}"
    local current_port=$(grep -E "^Port " /etc/ssh/sshd_config | awk '{print $2}')
    [ -z "$current_port" ] && current_port="22 (默认)"
    echo -e "当前端口: ${SKYBLUE}$current_port${PLAIN}"
    echo -e "${RED}注意: 若您的 VPS 有网页防火墙 (如 Oracle/AWS 安全组)，请务必先在那里放行新端口！${PLAIN}"
    read -p "是否修改 SSH 端口? (y/n): " mod_port_opt
    
    if [[ "$mod_port_opt" == "y" ]]; then
        read -p "请输入新的端口号 (1024-65535): " new_port
        if [[ "$new_port" =~ ^[0-9]+$ ]] && [ "$new_port" -ge 1024 ] && [ "$new_port" -le 65535 ]; then
             # 备份配置
             [ ! -f /etc/ssh/sshd_config.bak_port ] && cp /etc/ssh/sshd_config /etc/ssh/sshd_config.bak_port
             
             # 修改端口
             sed -i '/^Port /d' /etc/ssh/sshd_config
             echo "Port $new_port" >> /etc/ssh/sshd_config
             
             # 尝试放行防火墙 (UFW/Iptables)
             if command -v ufw >/dev/null 2>&1 && ufw status | grep -q "Status: active"; then
                ufw allow $new_port/tcp
                echo -e "${GREEN}✅ UFW 规则已更新: Allow $new_port/tcp${PLAIN}"
             fi
             
             # 如果有 iptables 且不是 UFW 管理的，手动添加规则
             if command -v iptables >/dev/null 2>&1; then
                # 简单判断是否为 UFW 托管，如果不是则添加 Input 规则
             ...
             echo -e "${RED}重要提示: 请勿立即断开当前连接！${PLAIN}"
             echo -e "请新开一个终端窗口尝试: ${YELLOW}ssh -p $new_port root@$(curl -s ifconfig.me)${PLAIN} 验证连接。"
        else
             echo -e "${RED}输入无效 (需为 1024-65535 的数字)，跳过修改。${PLAIN}"
        fi
    fi
