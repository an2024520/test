#!/bin/bash
# ===== 兼容 bash <(curl ...) 或 source 方式运行（解决 local 只能在函数内使用的问题）=====
if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
    # 被 source 或进程替换方式运行时，将整个脚本包裹进一个函数执行
    _pipeline_compat_main() {
        # 原脚本全部内容从这里开始（保持原样，仅增加缩进）
...
repair_icmp9_priority() {
    echo -e "${YELLOW}正在修复 ICMP9 优先级冲突...${PLAIN}"
    ...
    # 逻辑：将含有 "user" 字段的规则（ICMP9 特征）强制移动到路由规则列表顶部
    # 原理：Xray 路由匹配是顺序执行，IPV6 补丁插入的 ::/0 规则会导致下方基于 user 的转发失效
    if jq '
        .routing.rules |= sort_by(if has("user") then 0 else 1 end)
    ' "$CONFIG_FILE" > "$tmp"; then
        mv "$tmp" "$CONFIG_FILE"
        systemctl restart xray
        echo -e "${GREEN}修复完成！ICMP9 规则已置顶。${PLAIN}"
        echo -e "${SKYBLUE}建议重新测试中转连通性。${PLAIN}"
    else
...
while true; do
    ...
    st="${RED}未配置${PLAIN}"
    has_cred=false
    if [[ -f "$CRED_FILE" ]] && grep -q "PRIV_KEY" "$CRED_FILE" 2>/dev/null; then
        has_cred=true
    fi
    ...
    echo -e "============================================"
    echo -e " Xray IPv6 优先 + WARP 兜底补丁 (v5.1 Ultimate Final)"
    echo -e " 当前状态: [$st]"
...
# 如果是正常直接执行（./script.sh 或 bash script.sh），则直接运行原逻辑（无需包裹）
