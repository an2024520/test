...
menu_xray() {
    while true; do
        ...
        case "$xray_choice" in
            ...
            5)
                # 卸载 Xray
                check_run "$FILE_XRAY_UNINSTALL" "$URL_XRAY_UNINSTALL"
                ;;
            0)
                break 
                ;;
...
while true; do
    ...
    echo -e "    ${GRAY}- 进入 Xray 专属管理界面 (Classic Menu)${PLAIN}"
    echo -e ""
