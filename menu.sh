...
# 动态生成 Xray 卸载脚本 (如果不存在)
ensure_xray_uninstall_script() {
    if [[ ! -f "$FILE_XRAY_UNINSTALL" ]]; then
        cat <<EOF > "$FILE_XRAY_UNINSTALL"
#!/bin/bash
RED='\033[0;31m'
GREEN='\033[0;32m'
PLAIN='\033[0m'
...
menu_xray() {
    while true; do
        ...
        case "$xray_choice" in
            ...
            9) ensure_xray_uninstall_script; ./$FILE_XRAY_UNINSTALL; read -p "按回车继续..." ;;
            0) break ;;
...
while true; do
    ...
    echo -e "    ${GRAY}- VLESS / Vision / XHTTP / Reality${PLAIN}"
    echo -e ""
